...
func flash(node: CanvasItem, color: Color = Color.WHITE, duration: float = 0.1) -> void:
...
func float_text(text: String, position: Vector2, parent: Node, color: Color = Color.WHITE, duration: float = 0.8) -> void:
	var label := Label.new()
	label.text = text
